...
```{python py-linreg-setup, eval=FALSE}
import numpy as np
import pandas as pd
...
result_sm_ols = model.fit()

pd.DataFrame(
  [
  np.append(result_ml.x[0]**2, result_ml.x[1:]), 
  np.append(result_ls.fun/(N - X_mm.shape[1] - 1), result_ls.x), 
  np.append(result_sm_ols.scale, result_sm_ols.params)
  ], 
