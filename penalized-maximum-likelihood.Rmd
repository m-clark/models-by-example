...
A standard regression model via penalized likelihood.  See the [linear
regression example][Linear Regression] code for comparison. Here the
penalty is specified (via lambda argument) but one would typically estimate via
cross-validation or some other fashion. Two penalties are possible with the
...
procedure. I have more technical approaches to the lasso and ridge in the [lasso][L1 (lasso) regularization] and [ridge][L2 (ridge) regularization] chapters.
