...
Note that this will likely take a while, but you can bump the iterations down or decrease the sample size and get roughly the same estimates.

```{r bayes-cfa-est, results = 'hide', cache.rebuild=FALSE}
stan_data = 
  list(
...
```{r bayes-cfa-compare1, cache.rebuild=T}
print(
  fit_cfa,
  digits = 3,
  par    = c('lambda', 'phi'),
...
```{r bayes-compare-loadings, echo = FALSE}
lav_par %>% 
  filter(op == '=~') %>%
...
```{r bayes-compare-cor, echo = FALSE}
lav_cors = lav_par %>% 
