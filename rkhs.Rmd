...
We can use `base::solve`, though this function avoids a computationally singular result.
...
gcv_search = lapply(lambda, function(lam) ridge(X, y, lam))
...
I add a comparison to glmnet, where `alpha = 0` is equivalent to ridge regression.
...
gcv_search = lapply(lambda, function(lam) smoothing_spline(x, y, lam))
...
I've added comparison to an additive model using <span class="mgcv" style = "">mgcv</span>. Compare the result to Figure 2 of the Supplementary Material.

```{r rk-spline-comparison}
